...
class PacmanRules:
    ...
    def consume(position, state):
        ...
        if state.data.food[x][y]:
            state.data.scoreChange += settings.FOOD_REWARD
...
def readCommand(argv):
    ...
    parser.add_option(
        "-n", "--numGames", dest="numGames", type="int", help=default("the number of GAMES to play"), metavar="GAMES", default=1
    )
    parser.add_option(
        "-l",
        "--layout",
    ...
    parser.add_option(
        "-t", "--textGraphics", action="store_true", dest="textGraphics", help="Display output as text only", default=False
    )
    parser.add_option(
        "-q",
        "--quietTextGraphics",
    ...
    parser.add_option(
        "-k", "--numghosts", type="int", dest="numGhosts", help=default("The maximum number of ghosts to use"), default=4
    )
    parser.add_option(
    ...
    parser.add_option(
        "-a", "--agentArgs", dest="agentArgs", help='Comma separated values sent to agent. e.g. "opt1=val1,opt2,opt3=val3"'
    )
    parser.add_option(
        "-x",
        "--numTraining",
